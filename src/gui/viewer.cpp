...
void Viewer::update_display_background(cv::Mat desiredBackground) {
    ...
    //cv::Mat f0;
    //cv::Mat f1;
    //cv::flip(desiredBackground, f0, 0); // Flip U/D
    //cv::flip(f0, f1, 1); // Flip L/R
    //desiredBackground=f1.clone();
    
    // Perform flipping
    cv::Mat f0;
    cv::flip(desiredBackground, f0, 1); // Flip U/D
    desiredBackground=f0.clone();
...
void Viewer::update_display_background_to_original_image(int frame_number,
    ...
    if (CameraASelected) {
        //cv::Mat my_mat=loaded_frames_[frame_number].GetOriginalImage();
        //cv::imshow("A", my_mat);
        //cv::waitKey(0);
    } else {
        //cv::Mat my_mat=loaded_frames_B_[frame_number].GetOriginalImage();
        //cv::imshow("B", my_mat);
        //cv::waitKey(0);
    }
...
void Viewer::setup_camera_calibration(Calibration cal) {
    ...
    if (cal.type_ == "UF") {
        ...
        background_camera_->SetClippingRange(100.0, 5000.0);
    }
}
...
void Viewer::place_image_actors_according_to_calibration(Calibration cal,
    ...
    if (use_cam_A) {
        ...
        (cal.type_ == "UF") ? z_pos = -cal.camera_B_principal_.fy() *cal
                                       .camera_B_principal_.pixel_pitch_
                        : z_pos = cal.camera_B_principal_.fy();
    }
...
void Viewer::set_vtk_camera_from_calibration_and_image_if_camera_matrix(
    Calibration cal, int w, int h, bool use_cam_A) {
    ...
    if (use_cam_A) {
        cx = cal.camera_A_principal_.cx();
        cy = cal.camera_A_principal_.cy();
        fx = cal.camera_A_principal_.fx();
        fy = cal.camera_A_principal_.fy();
    } else {
        cx = cal.camera_B_principal_.cx();
        cy = cal.camera_B_principal_.cy();
        fx = cal.camera_B_principal_.fx();
        fy = cal.camera_B_principal_.fy();
    }
    
    calculate_and_set_window_center_from_calibration(w, h, cx, cy);
    update_window_center_on_resize();
    calculate_and_set_viewing_angle_from_calibration(h, fy);
    calculate_and_set_camera_aspect_from_calibration(fx, fy);
    scene_camera_->SetClippingRange(0.1 * fx, 1.75 * fx);
    scene_camera_->SetViewUp(0, -1, 0);
    
    //std::cout << "Updated scene cam" << "\n";
    //std::cout << use_cam_A << " " << fx << " " << fy << " " << cx << " " << cy << "\n";
    
    //double wc[2];
    //scene_camera_->GetWindowCenter(wc);
    //std::cout << "Window center" << "\n";
    //std::cout << wc[0] << " " << wc[1] << "\n";
...
int Viewer::get_num_loaded_models() {
    return this->num_models_loaded_;
}
