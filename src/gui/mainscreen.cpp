...
//////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////
void MainScreen::update_background_camera_config(Calibration my_cal, bool use_cam_A) {
    
    
    // Wrapper method that consolidates all methods pertaining to updating the camera configuration
    
    ...
    if (my_cal.type_=="UF") {
        //
            ...
            // DU/XMALAB biplanar cal
            
            // https://gist.github.com/decrispell/fc4b69f6bedf07a3425b
            
            // Coordinate system from XMA lab is x-right, y-down, z-forward
            renderer->GetActiveCamera()->SetPosition(0, 0, 0);
            //coronal_renderer->GetActiveCamera()->SetPosition(0, 0, 0);
            
            // Look in the +Z direction of the camera coordinate system
            renderer->GetActiveCamera()->SetFocalPoint(0, 0, 1);
            //coronal_renderer->GetActiveCamera()->SetFocalPoint(0, 0, 1);
            
            // The camera Y axis points down
            //renderer->GetActiveCamera()->SetViewUp(0,-1,0); // Not needed bc open image imported as FLIPPED

            // Ensure the relevant range of depths are rendered
            float depth_min=100.0;
            float depth_max=5000.0;
            renderer->GetActiveCamera()->SetClippingRange(depth_min, depth_max);
            //coronal_renderer->GetActiveCamera()->SetClippingRange(depth_min, depth_max);
        
            if (use_cam_A) {
                // Convert the principal point to window center (normalized coordinate system) and set it
                //double wcx =  2*(calibration_file_.camera_A_principal_.cx() - double(calibration_file_.camera_A_principal_.im_w_)/2) / calibration_file_.camera_A_principal_.im_w_;
                //double wcy = -2*(calibration_file_.camera_A_principal_.cy() - double(calibration_file_.camera_A_principal_.im_h_)/2) / calibration_file_.camera_A_principal_.im_h_;
                //renderer->GetActiveCamera()->SetWindowCenter(wcx, wcy);
                //coronal_renderer->GetActiveCamera()->SetWindowCenter(wcx, wcy);
                // Principal point defined wrt viewport coords: [-1,+1],[-1,+1], Y-axis up, X-axis right, Z-axis forward
                // https://book.vtk.org/en/latest/VTKBook/03Chapter3.html
                // Also see function reference at:
                // https://vtk.org/doc/nightly/html/classvtkCamera.html#acab55ca85879f5d9cb3c93a72319759a

                // Convert the focal length to view angle and set
                // Use average of fx and fy for approximation, as only single value supported
                //float f_viz=(calibration_file_.camera_A_principal_.fx()+
                //             calibration_file_.camera_A_principal_.fy())/2.0;
                //double view_angle = 180.0/3.1415 * (2.0 * std::atan2( ((double)calibration_file_.camera_A_principal_.im_h_)/2.0, f_viz ));
                //renderer->GetActiveCamera()->SetViewAngle(view_angle);
                //coronal_renderer->GetActiveCamera()->SetViewAngle(view_angle);
                // Also see SetViewAngle ref at:
                // https://vtk.org/doc/nightly/html/classvtkCamera.html#acab55ca85879f5d9cb3c93a72319759a
            } else {
                //double wcx =  2*(calibration_file_.camera_B_principal_.cx() - double(calibration_file_.camera_B_principal_.im_w_)/2) / calibration_file_.camera_B_principal_.im_w_;
                //double wcy = -2*(calibration_file_.camera_B_principal_.cy() - double(calibration_file_.camera_B_principal_.im_h_)/2) / calibration_file_.camera_B_principal_.im_h_;
                //renderer->GetActiveCamera()->SetWindowCenter(wcx, wcy);
                //coronal_renderer->GetActiveCamera()->SetWindowCenter(wcx, wcy);
                //float f_viz=(calibration_file_.camera_B_principal_.fx()+
                //             calibration_file_.camera_B_principal_.fy())/2.0;
                //double view_angle = 180.0/3.1415 * (2.0 * std::atan2( ((double)calibration_file_.camera_B_principal_.im_h_)/2.0, f_viz ));
                //renderer->GetActiveCamera()->SetViewAngle(view_angle);
                //coronal_renderer->GetActiveCamera()->SetViewAngle(view_angle);
            }
            ///////////////////////////////////////////////////////
...
void MainScreen::update_scene_camera_config(Calibration my_cal, bool use_cam_A) {

    // Update camera config for rendering STL models
    ...
    if (calibration_file_.type_ == "UF") {
        ...
        //     calibration_file_, loaded_frames[0].GetOriginalImage().cols,
        //     loaded_frames[0].GetOriginalImage().rows);
    } else { // if (calibration_file_.type_ == "Denver" || calibration_file_.type_ == "Denver2") {
    ...
    */
    ...
    if (true) { //(calibration_file_.type_ == "Denver2") {
        vw->set_vtk_camera_from_calibration_and_image_if_camera_matrix(
            calibration_file_, 
            loaded_frames[0].GetOriginalImage().cols,
            loaded_frames[0].GetOriginalImage().rows,
...
double MainScreen::CalculateViewingAngle(int width, int height, bool CameraA) {
    ...
    std::cout << "Warning: Calculate viewing angle called" << "\n";
...
void MainScreen::on_actionSave_Pose_triggered() {
    ...
    if (1==0) { // ONLY SAVES A SINGLE FRAME
        // Save Single Pose
        // Selection Check
        /*Load Models Selected Indices*/
        QModelIndexList selected =
...
void MainScreen::on_actionSegment_TibHR_triggered() {
    ...
    // REPURPOSED TO SELECT SEGMENTATION ARCH FOR CURRENT COMPONENT
    ...
    QString pt_model_location = QFileDialog::getOpenFileName(
        this, tr("Load Segmentation Architecture for Currently Selected Component"), ".",
        tr("Torch File (*.pt)"));
    if (pt_model_location.toStdString() != "") {
        segmentHelperFunction(pt_model_location.toStdString(), 1024, 1024);
    }
...
void MainScreen::update_image_list_widget() {
    ...
    // Method for actually updating background image data
...
void MainScreen::on_load_calibration_button_clicked() {
    ...
    if (inputFile.open(QIODevice::ReadOnly)) {
        ...
        */
        ...
        if (InputList[0] == "DU_INTCALIB_BIPLANE") {
        
            // Biplanar DU technique
            
            // Unpack info from input file
            double fx0=InputList[1].toDouble();
            double fy0=InputList[2].toDouble();
            double cx0=InputList[3].toDouble();
            double cy0=InputList[4].toDouble();
            double im0_w=InputList[5].toDouble();
            double im0_h=InputList[6].toDouble();
            
            double fx1=InputList[7].toDouble();
            double fy1=InputList[8].toDouble();
            double cx1=InputList[9].toDouble();
            double cy1=InputList[10].toDouble();
            double im1_w=InputList[11].toDouble();
            double im1_h=InputList[12].toDouble();
            
            double r00=InputList[13].toDouble();
            double r10=InputList[14].toDouble();
            double r20=InputList[15].toDouble();
            double r01=InputList[16].toDouble();
            double r11=InputList[17].toDouble();
            double r21=InputList[18].toDouble();
            double r02=InputList[19].toDouble();
            double r12=InputList[20].toDouble();
            double r22=InputList[21].toDouble();
            
            double bx=InputList[22].toDouble();
            double by=InputList[23].toDouble();
            double bz=InputList[24].toDouble();
            
            double s0=0;
            double s1=0;
            
            CameraCalibration DU_CAL0(fx0, s0, cx0, fy0, cy0, im0_w, im0_h);
            CameraCalibration DU_CAL1(fx1, s1, cx1, fy1, cy1, im1_w, im1_h);
            
            Vect_3 origin_B(bx, by, bz);
            Matrix_3_3 orthogonal_axes_B(r00, r01, r02,
                                         r10, r11, r12,
                                         r20, r21, r22);
            
            calibration_file_ = Calibration(DU_CAL0,
                                            DU_CAL1,
                                            origin_B, 
                                            orthogonal_axes_B);
    ...
    if (calibrated_for_monoplane_viewport_) {
        ...
        // USE BUTTON
        if (false){ //ui.image_list_widget->currentIndex().row() >= 0) {
    ...
    // Biplane Calibration
        ...
        if (calibration_file_.camera_A_principal_.type_=="Denver" ) // Generic biplanar cal
        {
            ...
            float f_viz=(calibration_file_.camera_A_principal_.fx()+
                         calibration_file_.camera_A_principal_.fy())/2.0;
            double view_angle = 180.0/3.1415 * (2.0 * std::atan2( ((double)calibration_file_.camera_A_principal_.im_h_)/2.0, f_viz ));
            std::cout << ((double)calibration_file_.camera_A_principal_.im_h_)/2.0 << " " << f_viz << " " << " " << "\n";
...
void MainScreen::on_load_image_button_clicked() {
    ...
    if (calibrated_for_monoplane_viewport_) {
        ...
        QStringList TiffFileExtensions = QFileDialog::getOpenFileNames(
            ...
            tr("Image File(s) (*.tif *.tiff *.png *.jpg *.jpeg)"));
    ...
    */
    ...
    stop_biplane:;
        ...
        // Upload frames to viewer object
        vw->set_loaded_frames(loaded_frames);
        
        //cv::Mat my_mat=loaded_frames[0].GetOriginalImage();
        //cv::imshow("this->imd0", my_mat);
        //cv::waitKey(0);
        //cv::Mat my_mat2=loaded_frames_B[0].GetOriginalImage();
        //cv::imshow("this->imd0", my_mat2);
        //cv::waitKey(0);
        
        // Upload frames to viewer object
        vw->set_loaded_frames_b(loaded_frames_B);
        
        // Update selected frames, thus updating the view
        if (ui.image_list_widget->currentRow() < 0 &&
            loaded_frames.size() > 0) {
            ui.image_list_widget->setCurrentRow(0);
        } 
    ...
    // Default to camera A, cannot return to camera A until first going to B
    ui.camera_A_radio_button->setDisabled(true);
...
void MainScreen::on_load_model_button_clicked() {
    ...
    this->update_scene_camera_config(calibration_file_, true);
...
void MainScreen::on_camera_A_radio_button_clicked() {
    ...
    if (ui.image_list_widget->currentIndex().row() >= 0) {
        ...
        //        .rows,
        //    true));
        this->update_background_camera_config(calibration_file_, true);
        ...
        if (selected.size() != 0 && !currently_optimizing_) {
            ...
            for (int r = 0; r < selected.size(); r++) {
                ...
                vw->set_model_position_at_index(
                    selected[r].row(), loaded_pose.x, loaded_pose.y,
                    loaded_pose.z);
                vw->set_model_orientation_at_index(
                    selected[r].row(), loaded_pose.xa, loaded_pose.ya,
                    loaded_pose.za);
                coronal_vw->set_model_position_at_index(
                    selected[r].row(), loaded_pose.x, loaded_pose.y,
                    loaded_pose.z);
...
void MainScreen::on_camera_B_radio_button_clicked() {
    ...
    if (ui.image_list_widget->currentIndex().row() >= 0) {
        ...
        this->update_background_camera_config(calibration_file_, false);
        ...
        this->update_scene_camera_config(calibration_file_, false);
...
void MainScreen::on_image_list_widget_itemSelectionChanged() {
    ...
    // Update cam cal data
    this->update_background_camera_config(calibration_file_, ui.camera_A_radio_button->isChecked());
    ...
    //} else {
        //vw->place_image_actors_according_to_calibration(
        //    calibration_file_,
        //    loaded_frames_B[this->curr_frame()].GetOriginalImage().rows,
        //    loaded_frames_B[this->curr_frame()].GetOriginalImage().cols);
        //coronal_vw->place_image_actors_according_to_calibration(
        //    calibration_file_,
        //    loaded_frames_B[this->curr_frame()].GetOriginalImage().rows,
        //    loaded_frames_B[this->curr_frame()].GetOriginalImage().cols);
