...
bool CostFunctionManager::initializeCorr(
...
bool CostFunctionManager::destructCorr(std::string &error_message) {
...
double CostFunctionManager::costFunctionCorr() {
    ...
    metric_score =
        gpu_metrics_->Corr(
             ...
             gpu_intensity_frames_A_->at(current_frame_index_)->GetGPUImage(),
    ...
    if (biplane_mode_) {
        ...
        gpu_metrics_->Corr(
             ...
             gpu_intensity_frames_B_->at(current_frame_index_)->GetGPUImage(),
             gpu_intensity_frames_B_->at(current_frame_index_)->GetInvertedGPUImage());
                
    } 
    
    double my_met=-1*(metric_score+metric_score2);
    //std::cout << my_met << "\n";
