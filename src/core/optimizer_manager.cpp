...
bool OptimizerManager::Initialize(
    ...
    //cv::Mat my_mat=frames_A_[0].GetOriginalImage();
    //cv::imshow("A", my_mat);
    //cv::waitKey(0);
    //cv::Mat my_mat2=frames_B_[0].GetOriginalImage();
    //cv::imshow("B", my_mat2);
    //cv::waitKey(0);
    
    cv::imwrite("frameA.png", frames_A_[0].GetOriginalImage());
    cv::imwrite("frameA_I.png", frames_A_[0].GetInvertedImage());
    if (calibration_.biplane_calibration)
    {
        cv::imwrite("frameB.png", frames_B_[0].GetOriginalImage());
        cv::imwrite("frameB_I.png", frames_B_[0].GetInvertedImage());
    }
    ...
    // NOTE: COULD STORE SEGMENTATION MAPS IN DILATION FRAMES
    ...
    for (int i = 0; i < frames_A_.size(); i++) {
        auto dilated_frame = new GPUDilatedFrame(
            ...
            leaf_dilation_val_);
        //auto dilated_frame = new GPUDilatedFrame(
        //    width, height, cuda_device_id, frames_A_[i].GetInvertedImage().data,
        //    leaf_dilation_val_);
    ...
    if (calibration_.biplane_calibration) {
        for (int i = 0; i < frames_B_.size(); i++) {
            ...
            //auto dilated_frame = new GPUDilatedFrame(
            //    width, height, cuda_device_id,
            //    frames_B_[i].GetInvertedImage().data, leaf_dilation_val_);
            if (dilated_frame->IsInitializedCorrectly()) {
    ...
    if (calibration_.biplane_calibration) {
        for (int i = 0; i < frames_B_.size(); i++) {
            ...
            //auto dilated_frame = new GPUDilatedFrame(
            //    width, height, cuda_device_id,
            //    frames_B_[i].GetInvertedImage().data, branch_dilation_val_);
            if (dilated_frame->IsInitializedCorrectly()) {
    ...
    if (calibration_.biplane_calibration) {
        for (int i = 0; i < frames_B_.size(); i++) {
            ...
            //auto dilated_frame = new GPUDilatedFrame(
            //    width, height, cuda_device_id,
            //    frames_B_[i].GetInvertedImage().data, trunk_dilation_val_);
            if (dilated_frame->IsInitializedCorrectly()) {
...
void OptimizerManager::Optimize() {
    ...
    std::cout << "Optimzing" << "\n";
    std::cout << !succesfull_initialization_ << "\n";
    ...
    std::cout << "writing final drr\n" <<"\n";
    gpu_principal_model_->RenderPrimaryCameraAndWriteImage(current_opt_pose, "opt1.png");
    //gpu_non_principal_model->RenderPrimaryCameraAndWriteImage(current_opt_pose, "opt2.png");
...
double OptimizerManager::EvaluateCostFunction(Point6D point) {
    ...
    if (result < current_optimum_value_) {
        ...
        std::cout << current_optimum_value_  << "\n";
    }
