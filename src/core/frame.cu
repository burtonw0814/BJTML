...
Frame::Frame(std::string file_location, int aperture, int low_threshold,
    ...
    flip(cv::imread(file_location, CV_8UC1), original_image_, 0); // Flip U/D
    //cv::imshow("A", original_image_);
    //cv::waitKey(0);
    //cv::Mat original2;
    //flip(original_image_, original2, 1); // Flip L/R
    //original_image_=original2;
    //cv::imshow("B", original_image_);
    //cv::waitKey(0);
    
    // Only flip at render time
    //original_image_=cv::imread(file_location, CV_8UC1);
